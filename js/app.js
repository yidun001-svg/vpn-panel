...
function switchPage(pageId) {
    ...
    const titles = { dashboard: '控制台', users: '用户管理', servers: '服务器管理', configs: '配置管理', subscription: '订阅管理', traffic: '流量统计', settings: '系统设置' };
    ...
    tbody.innerHTML = users.map(u => {
        ...
        return '<tr>' +
            ...
            '<td>' +
                ...
                '<button class="btn-icon copy" onclick="showSubscriptionUrl(\'' + u.id + '\')" title="订阅链接"><i class="fas fa-link"></i></button>' +
...
function renderSubscription() {
    ...
    // 渲染用户订阅列表
    renderSubscriptionUserList();
}

function renderSubscriptionUserList() {
    var container = document.getElementById('subscriptionUserList');
    if (!container) return;

    // 找出有配置的用户
    var usersWithConfigs = {};
    appData.configs.forEach(function(c) {
        if (!c.disabled) usersWithConfigs[c.userId] = true;
    });

    var activeUsers = appData.users.filter(function(u) {
        return usersWithConfigs[u.id] && getUserStatus(u) !== 'disabled';
    });

    if (activeUsers.length === 0) {
        container.innerHTML = '<p style="font-size:13px;color:var(--text-muted);padding:20px;text-align:center">暂无配置了代理的用户。请先在"配置管理"中为用户生成配置。</p>';
        return;
    }

    var baseUrl = document.getElementById('serverBaseUrl').value || 'http://localhost:3456';
    var html = '<div style="overflow-x:auto"><table class="data-table"><thead><tr>' +
        '<th>用户</th><th>订阅链接</th><th>配置数</th><th>操作</th></tr></thead><tbody>';

    activeUsers.forEach(function(u) {
        var configCount = appData.configs.filter(function(c) {
...
function exportClashYamlAll() {
    if (appData.configs.length === 0) {
        showToast('暂无配置，请先生成VPN配置', 'error');
        return;
    }

    var yaml = generateClashYaml(appData.configs, appData.users, appData.servers, null);

    // 弹窗显示
    var html = '<div class="form-group"><label>Clash YAML 配置</label>' +
        '<textarea readonly style="width:100%;height:350px;font-family:monospace;font-size:12px;background:var(--bg);border:1px solid var(--border);border-radius:var(--radius-sm);padding:12px;resize:vertical">' +
        escapeHtml(yaml) +
        '</textarea></div>' +
        '<button class="btn btn-primary" onclick="copyClashYamlAll()"><i class="fas fa-copy"></i> 复制到剪贴板</button>' +
        '<button class="btn btn-secondary" onclick="downloadClashYamlAll()" style="margin-left:8px"><i class="fas fa-download"></i> 下载 YAML</button>';

    // 保存最新生成的 YAML 用于复制/下载
    window._latestClashYaml = yaml;

    openModal('Clash YAML 配置导出', html);
    addAuditLog('导出Clash配置', '导出所有配置为Clash YAML');
}

function copyClashYamlAll() {
    if (!window._latestClashYaml) return;
    navigator.clipboard.writeText(window._latestClashYaml).then(function() {
        showToast('Clash配置已复制到剪贴板，可直接粘贴到Clash Verge中', 'success');
    }).catch(function() {
        showToast('复制失败，请手动选择文本复制', 'error');
    });
}

function downloadClashYamlAll() {
    if (!window._latestClashYaml) return;
    var blob = new Blob([window._latestClashYaml], { type: 'text/yaml;charset=utf-8' });
    var url = URL.createObjectURL(blob);
    var a = document.createElement('a');
    a.href = url;
    a.download = 'clash-config-' + new Date().toISOString().split('T')[0] + '.yaml';
    a.click();
    URL.revokeObjectURL(url);
    showToast('Clash配置文件已下载', 'success');
}

function exportClashYamlForUser(userId) {
    var user = appData.users.find(function(u) { return u.id === userId; });
    if (!user) return;

    var yaml = generateClashYaml(appData.configs, appData.users, appData.servers, userId);

    var html = '<div class="form-group"><label>用户：' + escapeHtml(user.name) + ' 的 Clash 配置</label>' +
        '<textarea readonly style="width:100%;height:350px;font-family:monospace;font-size:12px;background:var(--bg);border:1px solid var(--border);border-radius:var(--radius-sm);padding:12px;resize:vertical">' +
        escapeHtml(yaml) +
        '</textarea></div>' +
        '<button class="btn btn-primary" onclick="copyClashYamlAll()"><i class="fas fa-copy"></i> 复制</button>' +
        '<button class="btn btn-secondary" onclick="downloadClashYamlAll()" style="margin-left:8px"><i class="fas fa-download"></i> 下载</button>';

    window._latestClashYaml = yaml;

    openModal('Clash配置 - ' + escapeHtml(user.name), html);
...
function showSubscriptionUrl(userId) {
    ...
    if (configCount === 0) {
        showToast('该用户暂无有效配置，请在"配置管理"中生成', 'warning');
        return;
    }
...
function copySubscriptionUrl(userId) {
    var baseUrl = document.getElementById('serverBaseUrl') ? document.getElementById('serverBaseUrl').value : 'http://localhost:3456';
...
function syncDataToServer() {
    var baseUrl = document.getElementById('serverBaseUrl').value || 'http://localhost:3456';
    var btn = document.getElementById('btnSyncServer');
    ...
    .catch(function(err) {
        showToast('无法连接到订阅服务器 (' + baseUrl + ')，请先运行 node server.js', 'error');
        updateServerStatus(false);
    })
    .finally(function() {
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> 同步数据到服务器';
        }
    });
}
...
function checkServerStatus() {
    ...
    .then(function(data) {
        if (data.status === 'running') {
            updateServerStatus(true);
            showToast('订阅服务器已连接 (端口: ' + data.port + ', 配置数: ' + data.configs + ')', 'success');
        } else {
            updateServerStatus(false);
        }
    })
...
function updateServerStatus(online) {
    ...
    if (online) {
        ...
        statusEl.lastChild.textContent = ' 服务器已连接';
    } else {
        dot.className = 'status-dot offline';
        statusEl.lastChild.textContent = ' 服务器未连接';
    }
}
...
// ====== 数据迁移助手 ======
function hasAppData() {
    return appData.users.length > 0 || appData.servers.length > 0 || appData.configs.length > 0;
}

function showMigrationGuide() {
    // 仅在 HTTP 模式下且无数据时显示
    if (!window.location.protocol.startsWith('http') || hasAppData()) return;

    var html = '<div style="padding:8px 0">' +
        '<div style="background:var(--primary-light);border:1px solid var(--primary);border-radius:var(--radius-sm);padding:16px;margin-bottom:16px">' +
        '<p style="font-weight:bold;color:var(--primary);margin-bottom:8px"><i class="fas fa-info-circle"></i> 首次使用？从旧版迁移数据</p>' +
        '<p style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">你的数据存在之前双击 index.html 打开的版本里，需要迁移过来：</p>' +
        '<ol style="font-size:13px;color:var(--text-secondary);padding-left:20px;line-height:2">' +
        '<li>在文件管理器中双击 <code style="background:var(--bg);padding:2px 6px;border-radius:3px">index.html</code> 打开旧版</li>' +
        '<li>点击左侧 <b>系统设置</b> → 点击 <b>导出数据</b>，保存 JSON 文件</li>' +
        '<li>回到本页面 → <b>系统设置</b> → 点击 <b>导入数据</b>，选择刚才的 JSON 文件</li>' +
        '</ol>' +
        '<p style="font-size:12px;color:var(--text-muted)">只需操作一次，之后通过本页面使用即可，数据会自动同步到服务器。</p>' +
        '</div></div>';

    document.querySelector('.page#page-dashboard .stats-grid').insertAdjacentHTML('beforebegin',
        '<div class="card migration-card" style="margin-bottom:16px;border-left:4px solid var(--primary)">' +
        '<div style="display:flex;justify-content:space-between;align-items:flex-start">' +
        '<div style="flex:1">' + html.replace('<div style="padding:8px 0">', '').replace('</div></div>', '') + '</div>' +
        '<button onclick="this.parentElement.parentElement.remove()" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--text-muted);padding:0 4px">&times;</button>' +
        '</div></div>'
    );
}
...
// ====== 初始化 ======
function init() {
    // 如果通过 HTTP 访问且本地无数据，尝试从服务器加载
    var isHttp = window.location.protocol.startsWith('http');
    var hasLocalData = appData.users.length > 0 || appData.servers.length > 0 || appData.configs.length > 0;

    if (isHttp && !hasLocalData) {
        var baseUrl = window.location.protocol + '//' + window.location.host;
        fetch(baseUrl + '/api/data')
            .then(function(res) { return res.json(); })
            .then(function(serverData) {
                if (serverData && (serverData.users.length > 0 || serverData.servers.length > 0)) {
                    for (var key in defaultData) {
                        if (serverData[key]) appData[key] = serverData[key];
                    }
                    saveData();
                    showToast('已从服务器加载 ' + serverData.users.length + ' 个用户, ' + serverData.servers.length + ' 台服务器', 'success');
                }
                finishInit();
            })
            .catch(function() {
    ...
    function finishInit() {
        ...
        // 数据迁移提示：HTTP 模式下无数据显示迁移引导
        if (isHttp && !hasAppData()) {
            setTimeout(showMigrationGuide, 800);
        }
